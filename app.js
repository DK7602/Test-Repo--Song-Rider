...
function setAuthMode(mode){
  const allowed = new Set(["login","reset-request","reset-password"]);
  cloud.authMode = allowed.has(mode) ? mode : "login";
}

function getAuthMode(){
  return cloud.authMode || "login";
}

function getAuthResetRedirectUrl(){
  try{
...
function updateAuthModalUI(message=""){
  ...
  const recoveryMode = !!cloud.passwordRecovery;
  const signedIn = !!cloud.user && !recoveryMode;
  const mode = recoveryMode ? "reset-password" : (signedIn ? "signed-in" : getAuthMode());

  modal.classList.toggle("signed-in", signedIn);
  modal.classList.toggle("signed-out", !signedIn && mode === "login");
  modal.classList.toggle("reset-request", !signedIn && mode === "reset-request");
  modal.classList.toggle("reset-password", !signedIn && mode === "reset-password");
  ...
  const backToRequestBtn = document.getElementById("authBackToRequestBtn");
  ...
  if(status){
    if(message){
      ...
      status.textContent = cloud.passwordRecovery
        ? "Choose a new password for your account."
        : "Projects sync privately to your signed-in account.";
    }else if(mode === "reset-request"){
      status.textContent = "Enter your email and we’ll send you a password reset link.";
    }else if(mode === "reset-password"){
      status.textContent = "Enter your new password below.";
    }else{
      status.textContent = "Sign in so your projects follow you across devices.";
    }
  }

  if(userEmail) userEmail.textContent = cloud.user ? (cloud.user.email || cloud.user.id || "Signed in") : "";

  [syncBtn, signOutBtn, signInBtn, signUpBtn, sendResetBtn, updatePasswordBtn, backToSignInBtn, backToRequestBtn, forgotBtn].forEach(btn => {
    if(btn) btn.disabled = !!cloud.authBusy;
  });
  [emailInput, passInput, resetEmailInput, newPasswordInput, confirmPasswordInput].forEach(inp => {
    if(inp) inp.disabled = !!cloud.authBusy;
  });
}

function openAuthModal(message="", mode=null){
  const modal = document.getElementById("authModal");
  if(!modal) return;
  if(!cloud.user && mode) setAuthMode(mode);
...
function closeAuthModal(){
  ...
  if(!cloud.user && !cloud.passwordRecovery) setAuthMode("login");
}
...
async function handleAuthSessionChange(session, reason=""){
  ...
  cloud.passwordRecovery = (reason === "PASSWORD_RECOVERY");

  if(cloud.passwordRecovery){
    setAuthMode("reset-password");
    openAuthModal("Choose a new password for your account.", "reset-password");
  }
...
async function submitPasswordResetEmail(){
  if(!cloud.client) await initCloudSync();
  ...
  try{
    ...
    if(error) throw error;
    updateAuthModalUI("Password reset link sent. Check your email.");
    setAuthMode("login");
  }catch(err){
    updateAuthModalUI(err?.message || "Could not send reset link.");
  }finally{
    setAuthBusy(false);
  }
}
...
async function submitUpdatedPassword(){
  ...
  const confirm = String(document.getElementById("authConfirmPasswordInput")?.value || "");
  if(!password || !confirm){
    updateAuthModalUI("Enter and confirm your new password.");
    return;
  }
  if(password.length < 6){
    updateAuthModalUI("Use a password with at least 6 characters.");
  ...
  if(password !== confirm){
    updateAuthModalUI("Passwords do not match.");
    return;
  }
  try{
    ...
    setAuthMode("login");
    updateAuthModalUI("Password updated.");
    closeAuthModal();
  }catch(err){
...
function wireAuthModal(){
  ...
  document.getElementById("authForgotPasswordBtn")?.addEventListener("click", () => {
    ...
    if(resetEmail && email && !resetEmail.value) resetEmail.value = email;
  ...
  document.getElementById("authBackToRequestBtn")?.addEventListener("click", () => {
    setAuthMode(cloud.passwordRecovery ? "reset-password" : "reset-request");
    updateAuthModalUI();
  });
  document.getElementById("authSendResetBtn")?.addEventListener("click", submitPasswordResetEmail);
  document.getElementById("authUpdatePasswordBtn")?.addEventListener("click", submitUpdatedPassword);
  ...
  document.getElementById("authResetEmailInput")?.addEventListener("keydown", (e) => {
    if(e.key === "Enter") submitPasswordResetEmail();
  });
  ["authNewPasswordInput", "authConfirmPasswordInput"].forEach(id => {
    document.getElementById(id)?.addEventListener("keydown", (e) => {
      if(e.key === "Enter") submitUpdatedPassword();
    });
  });
