...
/***********************
Chord Tracker v2 — live mic note + chord detection
...
const CHORD_TRACKER_FRAME_KEEP_MS = 1250;
const CHORD_TRACKER_STAMP_COOLDOWN_MS = 520;
const CHORD_TRACKER_UI_MS = 110;
...
function chordTrackerAggregate(){
  ...
  for(const fr of frames){
    const age = Math.max(0, nowMs - (Number(fr.t)||nowMs));
    // Recent frames count more, but older ringing strings still matter for guitar chords.
    const recency = clamp(1 - (age / CHORD_TRACKER_FRAME_KEEP_MS), 0.18, 1);
    const w = 0.35 + recency * 0.65;

    if(Array.isArray(fr.chroma)){
      chromaWeight += w;
      for(let i=0;i<12;i++) chroma[i] += (Number(fr.chroma[i]) || 0) * w;
    }
    if(Number.isFinite(fr.pc)){
      const pc = ((fr.pc % 12) + 12) % 12;
      // Pitch detector is usually the fundamental/lowest ringing note; use it as a root hint,
      // not as enough evidence by itself.
      noteVotes[pc] += Math.max(0.04, Number(fr.pitchConfidence)||0.04) * w;
    }
  }

  if(chromaWeight > 0){
    for(let i=0;i<12;i++) chroma[i] /= chromaWeight;
  }

  const maxVote = Math.max(...noteVotes);
  const noteVotesNorm = Array(12).fill(0);
  if(maxVote > 0){
    for(let i=0;i<12;i++){
      noteVotesNorm[i] = noteVotes[i] / maxVote;
      // Slight blend only. Too much note vote makes G/Em and C/Am jump incorrectly.
      chroma[i] += 0.22 * noteVotesNorm[i];
    }
  }
  ...
  return {
    chroma,
    note,
    noteVotes: noteVotesNorm,
    noteConfidence: clamp(maxVote / Math.max(0.001, frames.length), 0, 1),
...
function chordTrackerChordTemplates(){
  return [
    ...
    { suffix:"5", pcs:[0,7], need:[0,7], label:"power" },
...
function chordTrackerInferChord(input){
  const agg = Array.isArray(input) ? { chroma: input, noteVotes: Array(12).fill(0) } : (input || {});
  const chroma = agg.chroma;
  const rootVotes = Array.isArray(agg.noteVotes) ? agg.noteVotes : Array(12).fill(0);
  ...
  const vote = (pc) => Number(rootVotes[((pc % 12) + 12) % 12]) || 0;

  const qualities = [
    { suffix:"", pcs:[0,4,7], third:4, opp:3, name:"major" },
    { suffix:"m", pcs:[0,3,7], third:3, opp:4, name:"minor" },
    { suffix:"sus2", pcs:[0,2,7], third:2, opp:4, altOpp:3, name:"sus2" },
    { suffix:"sus4", pcs:[0,5,7], third:5, opp:4, altOpp:3, name:"sus4" },
    { suffix:"7", pcs:[0,4,7,10], third:4, opp:3, ext:10, baseSuffix:"", name:"7" },
    { suffix:"m7", pcs:[0,3,7,10], third:3, opp:4, ext:10, baseSuffix:"m", name:"m7" }
  ];

  let best = null;

  for(let root=0; root<12; root++){
    const rootE = val(root);
    const fifthE = val(root+7);
    const rootHint = vote(root);

    for(const q of qualities){
      const abs = q.pcs.map(iv => (root + iv) % 12);
      const toneVals = abs.map(pc => val(pc));
      const avgTone = toneVals.reduce((a,b)=>a+b,0) / toneVals.length;
      const minTone = Math.min(...toneVals);
      const thirdE = val(root + q.third);
      const oppE = val(root + q.opp);
      const altOppE = Number.isFinite(q.altOpp) ? val(root + q.altOpp) : 0;
      const outside = chroma.reduce((sum, v, pc) => sum + (abs.includes(pc) ? 0 : Math.max(0, Number(v)||0)), 0) / 12;
      ...
      const isMinor = q.suffix === "m" || q.suffix === "m7";
      const isMajor = q.suffix === "" || q.suffix === "7";
      const isSus = q.suffix === "sus2" || q.suffix === "sus4";

      if(isMinor){
        // Minor thirds are quieter on many guitar recordings; give them a fair boost,
        // but reject if the major third is clearly stronger.
        score += 0.08;
        score -= Math.max(0, oppE - thirdE - 0.13) * 0.72;
      }
      if(isMajor){
        score -= Math.max(0, oppE - thirdE - 0.07) * 0.82;
      }
      if(isSus){
        score -= Math.max(oppE, altOppE) * 0.24;
      }
      if(q.ext && val(root + q.ext) < 0.34) score -= 0.18;

      // Common guitar ambiguity helpers: Em vs G, Am vs C, Dm vs F.
      // If the minor root is showing up as the stable pitch/root hint, let the minor win.
      if(isMinor && rootHint >= 0.42) score += 0.10;
      if(isMajor && vote(root-3) >= 0.45 && val(root-3) >= 0.20) score -= 0.10;

      const candidate = {
        root, suffix:q.suffix, score, avgTone, minTone, rootE, fifthE, thirdE,
        oppE: Math.max(oppE, altOppE), rootHint, pcs:abs, ext:q.ext, baseSuffix:q.baseSuffix || q.suffix
      };
  ...
  const strongCount = chroma.filter(v => Number(v) >= 0.36).length;
  const isMinor = best.suffix === "m" || best.suffix === "m7";
  const isMajor = best.suffix === "" || best.suffix === "7";
  const isSus = best.suffix === "sus2" || best.suffix === "sus4";

  const identityOk = (best.rootE >= 0.20 || best.fifthE >= 0.28 || best.rootHint >= 0.40 || best.avgTone >= 0.43);
  let thirdOk = true;
  if(isMinor){
    thirdOk = best.thirdE >= 0.18 && best.oppE <= best.thirdE + 0.18;
  }else if(isMajor){
    thirdOk = best.thirdE >= 0.20 && best.oppE <= best.thirdE + 0.12;
  }else if(isSus){
    thirdOk = best.thirdE >= 0.22;
  }

  const minScore = strongCount <= 1 ? 0.66 : 0.39;
  if(!identityOk || !thirdOk || best.avgTone < 0.28 || best.score < minScore) return null;

  let suffix = best.suffix;
  if(best.ext && val(best.root + best.ext) < 0.39){
    suffix = best.baseSuffix || (isMinor ? "m" : "");
  }

  const chord = chordTrackerCleanLabel(`${CHORD_TRACKER_ROOT_NAMES[best.root]}${suffix}`);
  const confidence = clamp((best.score + best.avgTone + best.thirdE + best.fifthE + (best.rootHint*0.4)) / 2.45, 0, 1);
...
function chordTrackerCurrentDetection(){
  ...
  if(chord && chord.confidence >= 0.40){
    return { type:"chord", value: chord.chord, confidence: chord.confidence, note: agg.note };
  }
  if(agg.note && agg.noteConfidence >= 0.40){
    return { type:"note", value: agg.note, confidence: agg.noteConfidence, note: agg.note };
  }
  return null;
}
...
function chordTrackerStabilizeDetection(det){
  ...
  if(!det || !det.value){
    ...
    if(locked && (t - (Number(state.chordTrackerLockedAt)||0) < 850)){
      return locked;
    }
    return null;
  }
  ...
  const needed = det.type === "chord" ? 3 : 4;
  ...
  const veryConfident = Number(det.confidence)||0 > (det.type === "chord" ? 0.66 : 0.72);
  ...
  const locked = state.chordTrackerLockedDetection;
  if(locked && (t - (Number(state.chordTrackerLockedAt)||0) < 700)) return locked;
  return null;
}
  ...
  try{
    ...
    if(analyser){
      ...
      if(det){
        ...
        if(det.value && det.confidence >= (det.type === "chord" ? 0.52 : 0.50)){
...
async function startChordTrackerMic(){
  ...
  try{
    ...
    state.chordTrackerCandidateConfidence = 0;
    state.chordTrackerLockedDetection = null;
    state.chordTrackerLockedAt = 0;
