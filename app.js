...
function chordTrackerLoop(){
  ...
  try{
    ...
    if(analyser){
      ...
      // STRICT MODE: use only the dedicated chord-tracker.js engine for live stamping.
      // The older in-app fallback was too willing to guess relative chords
      // (G -> Em, C -> Am, D -> F-ish). If the external engine is not loaded,
      // do not stamp anything rather than writing a wrong chord.
      let det = null;
      ...
      if(det){
        ...
        // Only stamp confirmed chords. Single-note detections are useful for the
        // status display, but they should not be written into chord boxes because
        // they look like wrong chords during piano triads.
        const minStamp = 0.66;
        if(det.type === "chord" && cleanVal && det.confidence >= minStamp){
