...
if (window.visualViewport) {
  ...
  const updateRhymeViewportUI = () => {
    ...
    // how much of the bottom of the layout viewport is covered (usually the keyboard)
    ...
    if(rBtn){
      // Keep the R button floating above the keyboard.
      // If the rhyme dock is open, float the button just above the dock too.
      const dockShown = !!(dock && dock.style.display === "block");
      const dockHeight = dockShown ? Math.ceil((dock.getBoundingClientRect()?.height || 0)) : 0;
      const extraLift = dockShown ? (dockHeight + 10) : 0;
      const totalLift = overlap + extraLift;

      rBtn.style.transform = `translateY(-${totalLift}px)`;
      rBtn.style.transition = "transform 160ms ease, opacity 160ms ease";
  ...
  // Re-run after the dock is toggled so the R button can sit above it.
  if(rBtn){
