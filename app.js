...
/***********************
Chord Tracker v5 — restored live detection + conservative lock-in
...
const CHORD_TRACKER_STAMP_COOLDOWN_MS = 560;
...
function chordTrackerAggregate(){
  ...
  if(maxVote > 0){
    for(let i=0;i<12;i++) chroma[i] += 0.32 * (noteVotes[i] / maxVote);
  }
  const max = Math.max(...chroma);
  if(max <= 0) return null;
...
function chordTrackerInferChord(input){
  ...
  if(!best) return null;
  const strongCount = chroma.filter(v => v >= 0.40).length;
  const thirdOk = best.suffix.startsWith("m") ? (best.thirdMinor >= 0.26) : (best.suffix === "" ? best.thirdMajor >= 0.26 : true);
  const minNeeded = strongCount <= 1 ? 0.66 : 0.34;
  const minScore = strongCount <= 1 ? 0.68 : 0.31;
  if(best.needed < minNeeded || best.score < minScore || !thirdOk) return null;

  // Do not overcall 7ths unless that 7th is really present.
  ...
  const confidence = clamp((best.score + best.needed + best.rootEnergy) / 2.05, 0, 1);
...
function chordTrackerCurrentDetection(){
  ...
  if(chord && chord.confidence >= 0.32){
  ...
  if(agg.note && agg.noteConfidence >= 0.20){
...
function chordTrackerStabilizeDetection(det){
  ...
  const needed = det.type === "chord" ? 2 : 1;
  ...
  try{
    ...
    if(analyser){
      ...
      if(det){
        ...
        if(stampVal && det.confidence >= (det.type === "chord" ? 0.30 : 0.18)){
...
function renderArchiveLoadedName(){
  // UI correction: do not show completed/archived project names beside the
  // Chord Tracker. The active project name now lives in the Project picker
  // window/label below, whether the project is active or archived.
