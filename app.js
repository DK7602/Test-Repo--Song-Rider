...
function chordTrackerInferChord(chroma){
  ...
  for(let root=0; root<12; root++){
    for(const tpl of templates){
      ...
      const hit = pcsAbs.reduce((sum, pc) => sum + (chroma[pc] || 0), 0) / Math.max(1, pcsAbs.length);
      const needed = needAbs.reduce((sum, pc) => sum + (chroma[pc] || 0), 0) / Math.max(1, needAbs.length);
      const outside = chroma.reduce((sum, v, pc) => sum + (pcsAbs.includes(pc) ? 0 : Math.max(0, v)), 0) / 12;
      const rootEnergy = chroma[root] || 0;
      const score = (hit * 0.55) + (needed * 0.38) + (rootEnergy * 0.20) - (outside * 0.55) - (pcsAbs.length > 3 ? 0.04 : 0);
      if(!best || score > best.score){
        best = { root, suffix:tpl.suffix, score, hit, needed, rootEnergy, pcs:pcsAbs };
      }
    }
  }

  if(!best) return null;
  const sorted = chroma.map((v,pc)=>({v,pc})).sort((a,b)=>b.v-a.v);
  const strongCount = sorted.filter(x => x.v >= 0.42).length;
  const minNeeded = best.suffix === "5" ? 0.52 : 0.44;
  const minScore = strongCount <= 1 ? 0.72 : 0.46;
  if(best.needed < minNeeded || best.score < minScore) return null;

  // Avoid overcalling extensions when a plain triad is clearer.
  ...
  if(["7","maj7","m7","add9","madd9"].includes(suffix)){
    ...
    if((chroma[extPc] || 0) < 0.42) suffix = suffix.startsWith("m") ? "m" : "";
  }

  const chord = `${CHORD_TRACKER_ROOT_NAMES[best.root]}${suffix}`;
  const confidence = clamp((best.score + best.needed + best.rootEnergy) / 2.35, 0, 1);
