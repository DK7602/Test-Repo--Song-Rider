...
(function(){
  ...
  function create(){
    ...
    const cfg = {
      ...
      minDb: -96,
      maxPeaks: 84,
      silenceRmsFloor: 0.0045,
      chordStableMs: 420,
      noteStableMs: 260,
      chordMinConfidence: 0.48,
      noteMinConfidence: 0.46
    };
    ...
    function pickSpectralPeaks(data, sampleRate, fftSize){
      ...
      if(!Number.isFinite(maxDb) || maxDb < -92) return [];
    ...
    function aggregate(){
      ...
      const strongNotes = chroma.map((v,i)=>({pc:i,v})).filter(x => x.v >= 0.26).sort((a,b)=>b.v-a.v);
    ...
    function inferChord(agg){
      ...
      const thirdOk = isMinor
        ...
        : (best.maj3 >= 0.20 && best.maj3 >= best.min3 * 1.03);
      if(!thirdOk) return null;

      // If the top two chord guesses are too close, refuse to guess.
      if(second && second.score >= best.score - 0.050) return null;

      // Relative major/minor chords share two notes. Refuse them unless the
      // defining third and root are clearly stronger. This specifically helps
      // stop G -> Em and C -> Am style mistakes on piano/phone microphones.
      const relativeRoot = best.suffix === '' ? (best.root + 9) % 12 : (best.root + 3) % 12;
      const relative = candidates.find(c => c.root === relativeRoot && c.suffix === (best.suffix === '' ? 'm' : ''));
      if(relative && relative.score >= best.score - 0.16){
        const definingThirdOk = best.suffix === 'm'
          ? best.min3 >= best.maj3 * 1.14
          : best.maj3 >= best.min3 * 1.10;
        const rootClearlyPresent = best.rootE >= 0.24;
        if(!definingThirdOk || !rootClearlyPresent) return null;
      }

      if(best.need < 0.30 || best.score < 0.30 || best.rootE < 0.18 || best.fifth < 0.16) return null;
    ...
    function inferNote(agg){
      ...
      if(best.v < 0.50 || agg.pitchConfidence < 0.24) return null;
    ...
    function stabilize(det, nowMs){
      ...
      const neededFrames = det.type === 'chord' ? 4 : 3;
    ...
    function analyze(analyser, audioCtx, nowMs){
      ...
      if(level < cfg.silenceRmsFloor || level < Math.max(cfg.silenceRmsFloor, state.noiseFloor * 1.20)){
