...
(function(){
  ...
  function create(){
    ...
    function inferChord(agg){
      ...
      if(best.weakest < 0.28) return null;

      const isMinor = best.suffix === 'm';
      const thirdOk = isMinor
        ? (best.min3 >= 0.30 && best.min3 >= best.maj3 * 1.12)
        : (best.maj3 >= 0.30 && best.maj3 >= best.min3 * 1.08);
      if(!thirdOk) return null;

      // If the top two chord guesses are too close, refuse to guess.
      if(second && second.score >= best.score - 0.105) return null;
      ...
      if(relative && relative.score >= best.score - 0.16){
        const definingThirdOk = best.suffix === 'm'
          ? best.min3 >= best.maj3 * 1.22
          : best.maj3 >= best.min3 * 1.18;
        const rootClearlyPresent = best.rootE >= 0.34;
        if(!definingThirdOk || !rootClearlyPresent) return null;
      }

      if(best.need < 0.42 || best.score < 0.41 || best.rootE < 0.26 || best.fifth < 0.24) return null;
    ...
    function stabilize(det, nowMs){
      ...
      const neededFrames = det.type === 'chord' ? 10 : 6;
